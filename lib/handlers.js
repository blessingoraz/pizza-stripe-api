...
/**
 * MENUITEM - POST
 * Required data: name, price, toppings
 * @param {*} data 
 * @param {*} callback
 */
handlers._menuItems.post = (data, callback) => {
    ...
    const price = typeof data.payload.price == 'number' && data.payload.price % 1 == 0 &&
        data.payload.price >= 200 && data.payload.price <= 500 ? data.payload.price : false
...
handlers._orders.post = (data, callback) => {
    ...
    if(email && menuId && size && quantity && stripeToken) {
        // Get the token from the headers
        const token = typeof data.headers.token == 'string' ? data.headers.token : false
                ...
                _data.read('menuItems', menuId, (err, menuItemData) => {
                    if(!err && menuItemData) {
                        ...
                        const orderObj = {
                            ...
                            quantity,
                            menu: menuItemData,
                            amount: menuItemData.price * quantity
                        }
                        ...
                        _data.create('orders', orderId, orderObj, (err) => {
                            if(!err) {
                                ...
                                helpers.makePayment(orderObj.amount, stripeToken, (err) => {
                                    if(!err) {
                                        // do somethong here
                                        console.log('Payment received')
                                        helpers.sendEmail(name, email, orderObj, (err) => {
                                            if(!err) {
                                                console.log('Email sent to user')
                                                callback(200)
                                            } else {
                                                callback(500, {'Error': 'Could not send email to user'})
                                            }
                                        })
                                    } else {
